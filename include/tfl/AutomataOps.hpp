...
namespace tfl {
    ...
    /**
     * @brief "Ghost class" containing functions to build \ref NFA/\ref DFA. 
     * 
     * @note Since DFA/NFA are implicitely convertible
     * into each other, all methods
    ...
    public:
        ...
        */
        static DFA<T>::Builder cross(typename DFA<T>::Builder const& left, typename DFA<T>::Builder const& right) {
            using StateIdx = DFA<T>::StateIdx;

            auto size = (left.state_count()+1) * (right.state_count()+1) - 1;
            auto compute_idx = cross_map(left, right);
            auto compute_ids = cross_remap(left, right);
        ...
        * @see \ref cross()
        */
        static auto cross_map(typename DFA<T>::Builder const& left, typename DFA<T>::Builder const& right) {
            using StateIdx = DFA<T>::StateIdx;
        ...
        * @see \ref cross()
        */
        static auto cross_remap(typename DFA<T>::Builder const& left, typename DFA<T>::Builder const& right) {
            using StateIdx = DFA<T>::StateIdx;
            auto lsize = left.state_count()+1;
            auto rsize = right.state_count()+1;
        ...
        */
        static NFA<T>::Builder literal(T const& t) {
        ...
        */
        static NFA<T>::Builder disjunction(typename NFA<T>::Builder const& left, typename NFA<T>::Builder const& right) {
        ...
        */
        static NFA<T>::Builder sequence(typename NFA<T>::Builder const& left, typename NFA<T>::Builder const& right) {
        ...
        */
        static NFA<T>::Builder closure(typename NFA<T>::Builder const& automaton) {
        ...
        */
        static DFA<T>::Builder complement(typename DFA<T>::Builder const& automaton) {
        ...
        */
        static DFA<T>::Builder conjunction(typename DFA<T>::Builder const& left, typename DFA<T>::Builder const& right) {
            auto builder = cross(left, right);
            auto compute_ids = cross_remap(left, right);
